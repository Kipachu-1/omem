...
- 110 tests, ~30s with model cached (OME-12 added per-client watermark: 3 memory_recent since:'lastSeen' tests + 1 resolveHttpClientName unit test). OME-17 added 9 (5 MCP `memory_recall` + 4 search `recall`/`noteMeta`) → 119 total. OME-15 added 5 `memory_usage`/observability tests (aggregation, error counting, OMEM_USAGE_LOG=off/json stderr) → 124 total. Tool surface is now eleven (added `memory_usage`).
...
- **MCP tools**: `src/mcp.ts` — eleven tools (added `memory_usage` in OME-15), registered on a fresh `McpServer` per HTTP request (stateless mode) or one server for stdio.
